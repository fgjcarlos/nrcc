	...
	t.Run("finds node-red container and reads state", func(t *testing.T) {
		c := &capturedCmd{
			psOutput: "aaa111\tredis:latest\tredis\tExited (0) 1 minute ago\t2026-01-01 00:00:00 +0000 UTC\n" +
				"bbb222\tnodered/node-red:4.1\tnrcc-node-red\tUp 5 minutes\t2026-01-02 00:00:00 +0000 UTC\n",
		...
		if info.Image != "nodered/node-red:4.1" {
			t.Fatalf("expected image nodered/node-red:4.1, got %q", info.Image)
		}
...
func TestDockerService_Status(t *testing.T) {
	...
	t.Run("returns full payload with container", func(t *testing.T) {
		c := &capturedCmd{
			psOutput:      "ddd444\tnodered/node-red:4.1\tnrcc-node-red\tUp 1 minute\t2026-03-01 00:00:00 +0000 UTC\n",
...
func TestDockerService_RestartAndStop(t *testing.T) {
	t.Run("restart runs docker restart on the discovered name", func(t *testing.T) {
		c := &capturedCmd{
			psOutput:      "eee555\tnodered/node-red:4.1\tnrcc-node-red\tUp 1 minute\t2026-04-01 00:00:00 +0000 UTC\n",
			inspectOutput: "true\tfalse\t0\n",
		}
		svc := newServiceWithDocker(t, c)
	...
	t.Run("stop runs docker stop on the discovered name", func(t *testing.T) {
		c := &capturedCmd{
			psOutput:      "fff666\tnodered/node-red:4.1\tnrcc-node-red\tUp 1 minute\t2026-04-01 00:00:00 +0000 UTC\n",
			inspectOutput: "true\tfalse\t0\n",
		}
		svc := newServiceWithDocker(t, c)
	...
	t.Run("action failure when docker CLI exits non-zero", func(t *testing.T) {
		c := &capturedCmd{
			psOutput:      "hhhh888\tnodered/node-red:4.1\tnrcc-node-red\tUp 1 minute\t2026-04-01 00:00:00 +0000 UTC\n",
			inspectOutput: "true\tfalse\t0\n",
