...
test.describe('Security Center transactional apply', () => {
  ...
  test('renders clear retry guidance for stale and readiness-failure transactions', async ({ page }) => {
    ...
    await page.getByLabel('Username').nth(0).fill('operator-updated')
    await page.getByRole('button', { name: 'Save Security Center' }).click()
    await expect(page.getByText('Settings changed elsewhere. Refresh, review the redacted preview, then retry.', { exact: true })).toHaveText(/Settings changed elsewhere.*Refresh.*retry/i)
    await page.getByRole('button', { name: 'Save Security Center' }).click()
    await expect(page.getByText('The transaction did not complete. Any failed readiness check is rolled back; review runtime readiness, then retry.', { exact: true })).toHaveText(/failed readiness check is rolled back.*retry/i)
  })
})
