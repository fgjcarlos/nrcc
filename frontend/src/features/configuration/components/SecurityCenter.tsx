...
export function SecurityCenter({ config, rawSettingsContent, expectedRevision, editable, onApplied }: Props) {
  ...
  return <section aria-labelledby="security-center-title" className="space-y-6">
    <div><h2 id="security-center-title" className="text-lg font-medium">{t('configuration:securityCenter.title')}</h2><p className="text-sm text-base-content/65">{t('configuration:securityCenter.subtitle')}</p></div>
    <aside aria-label={t('configuration:securityCenter.previewLabel')} className="rounded-xl border border-border bg-base-200/35 p-4 text-sm"><h3 className="font-medium">{t('configuration:securityCenter.previewTitle')}</h3><p>{t('configuration:securityCenter.previewBody')}</p></aside>
    ...
    {!editable && <p role="status" className="rounded-xl border border-warning/40 bg-warning/10 p-4 text-sm text-warning">{t('configuration:securityCenter.readOnly')}</p>}
    {editable && legacyAliases.length > 0 && <div className="rounded-xl border border-warning/40 bg-warning/10 p-4 text-sm"><h3 className="font-medium">{t('configuration:securityCenter.legacyTitle')}</h3><p>{t('configuration:securityCenter.legacyBody', { aliases: legacyAliases.join(' and ') })}</p></div>}
    {editable && <fieldset disabled={disabled} className="space-y-6">
      <div className="space-y-3"><div className="flex items-center justify-between"><h3 className="font-medium">{t('configuration:securityCenter.adminAuth')}</h3><button type="button" className="action-btn-secondary" onClick={() => { setChanged((current) => ({ ...current, admin: true })); setUsers((current) => [...current, emptyUser()]); }}>{t('configuration:securityCenter.addUser')}</button></div>
        <label className="block text-sm">{t('configuration:securityCenter.sessionExpiry')}<input aria-label="Session expiry seconds" className="input input-bordered mt-1 w-full" min="0" type="number" value={expiry} onChange={(event) => { setChanged((current) => ({ ...current, admin: true })); setExpiry(Number(event.target.value)); }} /></label>
        {users.map((user, index) => <div key={index} className="grid gap-3 rounded-xl border border-border p-3 md:grid-cols-4"><label className="text-sm">{t('configuration:securityCenter.username')}<input className="input input-bordered mt-1 w-full" value={user.username} onChange={(event) => updateUser(index, { username: event.target.value })} /></label><label className="text-sm">{t('configuration:securityCenter.permission')}<select aria-label={`Permission for ${user.username || `user ${index + 1}`}`} className="select select-bordered mt-1 w-full" value={user.permissions} onChange={(event) => updateUser(index, { permissions: event.target.value as '*' | 'read' })}><option value="*">{t('configuration:securityCenter.fullAccess')}</option><option value="read">{t('configuration:securityCenter.readOnlyPermission')}</option></select></label><label className="text-sm">{t('configuration:securityCenter.newPassword')}<input className="input input-bordered mt-1 w-full" type="password" value={user.password} onChange={(event) => updateUser(index, { password: event.target.value })} /></label><button type="button" className="btn btn-ghost self-end" aria-label={`Remove ${user.username || `user ${index + 1}`}`} onClick={() => { setChanged((current) => ({ ...current, admin: true })); setUsers((current) => current.filter((_, i) => i !== index)); }}>Remove</button></div>)}
      </div>
      <Surface title={t('configuration:securityCenter.nodeAuth')} value={node} onChange={(value) => { setChanged((current) => ({ ...current, node: true })); setNode(value); }} />
      <Surface title={t('configuration:securityCenter.staticAuth')} value={staticAuth} onChange={(value) => { setChanged((current) => ({ ...current, static: true })); setStaticAuth(value); }} />
      <button type="button" className="action-btn-primary" onClick={save} disabled={saving}>{t('configuration:securityCenter.save')}</button>
    </fieldset>}
...
function Surface({ title, value, onChange }: { title: string; value: BasicAuth; onChange: (value: BasicAuth) => void }) {
  const { t } = useT();
  return <div className="space-y-3"><h3 className="font-medium">{title}</h3><div className="grid gap-3 md:grid-cols-2"><label className="text-sm">{t('configuration:securityCenter.username')}<input className="input input-bordered mt-1 w-full" value={value.user} onChange={(event) => onChange({ ...value, user: event.target.value })} /></label><label className="text-sm">{t('configuration:securityCenter.bcryptHash')}<input className="input input-bordered mt-1 w-full" type="password" value={value.pass} onChange={(event) => onChange({ ...value, pass: event.target.value })} /></label></div><p className="text-sm text-base-content/65">{t('configuration:securityCenter.bcryptHint')}</p></div>;
}
